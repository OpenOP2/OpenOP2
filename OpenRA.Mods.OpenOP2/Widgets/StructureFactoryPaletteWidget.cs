...
{
	public class StructureFactoryPaletteWidget : Widget
	{
		...
		public ProductionQueue CurrentQueue
		{
			...
			set
			{
				...
				{
					cargoBay = currentQueue.Actor.TraitOrDefault<CargoBay>();
		...
		public StructureFactoryPaletteWidget(ModData modData, OrderManager orderManager, World world, WorldRenderer worldRenderer)
		{
		...
		public override void Tick()
		{
			...
			if (CurrentQueue != null)
			{
				var doneItems = CurrentQueue.AllQueued().Where(x => x.Done).ToList();
				foreach (var completedItem in doneItems)
				{
					if (cargoBay != null)
					{
						if (cargoBay.CanAddCargo())
						{
							cargoBay.AddCargo(completedItem.Item);
							CurrentQueue.EndProduction(completedItem);
						}
					}
				}
