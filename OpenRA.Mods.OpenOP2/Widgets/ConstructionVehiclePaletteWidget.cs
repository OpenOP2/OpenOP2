...
using OpenRA.Mods.OpenOP2.Orders;
...
{
	public class ConstructionVehicleIcon
	{
	...
	public class ConstructionVehiclePaletteWidget : Widget
	{
		...
		//public ConstructionVehicleIcon TooltipIcon { get; private set; }
		//public Func<ConstructionVehicleIcon> GetTooltipIcon;
		...
		public Actor CurrentActor
		{
			...
			set
			{
				...
				if (currentActor != null)
				{
					carriesCargo = CurrentActor.Trait<CarriesCargo>();
				}
		...
		Dictionary<Rectangle, ConstructionVehicleIcon> icons = new Dictionary<Rectangle, ConstructionVehicleIcon>();
		//Animation cantBuild;
		//Animation clock;
		...
		public ConstructionVehiclePaletteWidget(ModData modData, OrderManager orderManager, World world, WorldRenderer worldRenderer)
		{
		...
		{
			TotalIconCount = carriesCargo != null ? carriesCargo.Info.Capacity : 0;
		...
		bool HandleLeftClick(Actor actor, ConstructionVehicleIcon icon)
		{
			if (icon.Name == "eden-robo-dozer") return false;

			Game.Sound.PlayNotification(World.Map.Rules, World.LocalPlayer, "Sounds", ClickSound, null);

			if (actor != null)
			{
				var builderQueue = actor.TraitsImplementing<BuilderUnit>()
					.FirstOrDefault(q => q.Enabled);
				if (builderQueue != null)
				{
					World.OrderGenerator = new BuilderUnitBuildingOrderGenerator(builderQueue, icon.Name, worldRenderer);
				}
			}
		...
		bool HandleEvent(ConstructionVehicleIcon icon, MouseButton btn)
		{
		...
		{
			icons = new Dictionary<Rectangle, ConstructionVehicleIcon>();
			...
			if (CurrentActor != null)
			{
				...
				foreach (var item in carriesCargo.Bays.Where(bay => bay != null).Skip(IconRowOffset * Columns).Take(MaxIconRowOffset * Columns))
				{
					...
					var icon = new Animation(World, actorRenderSprites != null ? actorRenderSprites.Image : item);
					icon.Play("icon");

					var palette = "player";

					var pi = new ConstructionVehicleIcon()
					{
